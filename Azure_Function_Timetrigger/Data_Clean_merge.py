...
# Création de nouvelles données à partir du capteur Comsumption si l'information 
...
# Fonction permettant de faire des transformations adaptées au capteur Air
def Air_update(item, destination_container):
...
def cleaning_sensor(SensorName, item, destination_container) : 
    ...
    if SensorName in ["Consumption", "Desk", "Light", "Son", "TempEx"] : 
        ...
        if SensorName == "Consumption" :
            ...
            item.sort(key=lambda x: x['ReceivedTimeStamp'])
       ...
       Air_update(item, destination_container)
...
#     #b = 0
#     print(f"{sensor}")
#     name = input("Press touch command Enter: ")
